...
void writeValueToFile(const QString& filePath, const QString& value) {
    QFile file(filePath);
...
void SIGN_UP(sqlite3 *db ,string password ,string username ) {
    ...
    // Write a value
    writeValueToFile(filePath, QString::fromStdString(sha256(username)));
}
    ...
    string sql = "SELECT * FROM users WHERE username='" + username + "' AND password='" + hashedPassword + "';";
        ...
        if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, 0) == SQLITE_OK) {
            if (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                // Write a value
                writeValueToFile(filePath, QString::fromStdString(username));
                success = true;
            } else {
