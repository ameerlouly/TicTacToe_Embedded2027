...
pair<int, int> findBestMove(int board[3][3], Difficulty level) {
    ...
    // instead of returning {-1, -1} we return {0, 0} if there is no moves available
    //{-1, -1} craches the program because we changed some local variables to be global
    //have no idea what changed to cause a crash, but just leave it {0, 0}
